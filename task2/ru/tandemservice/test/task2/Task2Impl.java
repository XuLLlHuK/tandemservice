...
public class Task2Impl implements IElementNumberAssigner{
    ...
    private Task2Impl(){
    	try{
			nat = INSTANCE.getClass().getMethod("assignNumbers", List.class).
					getAnnotation(NumberAssignerTypeUse.class).value();
    ...
    public void assignNumbers(final List<IElement> elements) {
    	...
    	try{
    		if(elements == null || elements.isEmpty() || elements.size() == 0)
    			throw new EmptyListException("Пустой список.");
    		if(elements.size() == 1) {
    			System.out.println("Список состоит из одного элемента.");
    			return;
    		}
    		
			switch(nat){
				case FAST_SORT:
					new FastSortNumberAssigner().assignNumbers(elements);
					break;
				case SORTING:
				default:
					new SortNumberAssigner().assignNumbers(elements);
					break;
			}
