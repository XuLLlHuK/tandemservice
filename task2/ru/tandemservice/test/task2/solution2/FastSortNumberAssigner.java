...
public class FastSortNumberAssigner
{
	...
	{
		List<ElementWrapper> sortedElements = cloneAndSortList(
				elements);
		...
		for (IElement acceptor : elements)
		{
			...
			if (donor.getElement() != acceptor)
			{
				...
				if (donor.getElement().getNumber() == donor
						.getNumber())
	 ...
	 * @return
	 */
	private static List<ElementWrapper> cloneAndSortList(
			List<IElement> elements)
	{
		List<ElementWrapper> sortedList = new ArrayList<ElementWrapper>(
				elements.size());
